...
from extrautils import AttnLabelConverter
...
def create_text_read_envir(opt, doPrint=True):
...
def single_img(opt, frame, textToBeRead, converter, model, AlignCollate_demo, doPrint=True):
    # Expects frame to be from cv2 in RGB format
    demo_data = PredictedWebcamImage(frame, textToBeRead)  # use RawDataset
    ...
    with torch.no_grad():
        for image_tensors, image_path_list in demo_loader:
            ...
            if doPrint:
                print(f'{dashed_line}\n{"image_path":25s}\t{"predicted_labels":25s}\tconfidence score\n{"-" * 80}')
...
def batch_of_images(opt, frameDict, textToBeRead, converter, model, AlignCollate_demo, doPrint=True):
    demo_data = PredictedBatchOfImages(frameDict, textToBeRead)
    demo_loader = torch.utils.data.DataLoader(
        demo_data, batch_size=opt.batch_size,
        shuffle=False,
        num_workers=int(opt.workers),
